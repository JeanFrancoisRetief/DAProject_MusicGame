...
public class SideQuest005 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            hud.QuestTitleText.text = "Fetch Quest (_side_quest_005_StartCoroutine_Auto)";
            ObjectiveText = "Care packages collected " + packagesCollected.ToString() + " / " + packagesNeeded.ToString();
            hud.QuestObjectiveText.text = ObjectiveText;
            hud.TutorialText.text = tutText;

            if(QuestGiver.active == false)//dead
            {
                SpawnEndQuestTrigger();
            }

            if (packagesCollected == packagesNeeded)
            {
                packagesNeeded = packagesNeeded * 2;
                PlayDialogue();
                //Only one frame hopfully
            }
    ...
    public void EndQuest()
    {
        ...
        hud.WonQuestTitleText.text = "_side_quest_005_StartCoroutine_Auto == Fetch Quest.";
        hud.WonQuestObjectivesText.text = "Collected care packages\nKilled 999";
